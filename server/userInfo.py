...
class UserInfo(Resource):
        ...
        if user_name_i:
            ...
            if family_id:
                ...
                for info in family_id:
                    ...
                    family_info_vaccine_sql = "SELECT b.name from `get_vaccine` as a JOIN `vaccine` as b ON a.vaccine_id = b.id WHERE a.family_info_id = %s and a.get_vaccine = 1"
                    cursor.execute(family_info_vaccine_sql, (info[0], ))
    ...
    def post(self):
        ...
        if not params['name']:
            error = "There is no username info"
        elif not params['birth']:
            error = "There is no birth info"
        elif not params['gender']:
            error = "There is no gender info"
    ...
    def put(self):
        ...
        if family_info != []:
            for family in family_info:
                # 로그인한 user의 기존 family_info가 존재한다면 이름으로 특정 가족의 family_id를 받아오기
                family_sql = "SELECT `id` FROM `family_info` WHERE `user_info_id` = %s AND `name` = %s"
                cursor.execute(family_sql, (user_id, family['name']))
                ...
                if family_id:
                    family_info_update_sql = "UPDATE `family_info` SET `name` = %s, `birthday` = %s, `gender` = %s WHERE `user_info_id` = %s and `family_info_id` = %s"
                    cursor.execute(family_info_update_sql, (family['name'], family['birth'], family['gender'], user_id, family_id[0]))
                    conn.commit()
